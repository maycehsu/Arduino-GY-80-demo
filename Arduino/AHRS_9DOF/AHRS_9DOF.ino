...
void loop()
{
  ...
  if(!synched){
    ...
    while(Serial.available() > 0){
      inChar = Serial.read();
      if(inChar != -1 && inCharCount<16-1){
        inData[inCharCount++] = inChar;
        inData[inCharCount]=0;
      }
      else{
        break;
      }
    ...
    if(strlen(inData)>0){
      ...
      if(Serial.find!strcmp(inData,"#ob")){
        //errorPrintln("Turn on binary output");
        binaryOutput = 1;
      }
      else if(!strcmp(inData,"#o1")){
        //errorPrintln("Turn on continuous streaming output");
        continueOutput = 1;
      }
      else if(!strcmp(inData,"#o0")){
        //errorPrintln("Turn off continuous streaming output");
        continueOutput = 0;
      }
      else if(!strcmp(inData,"#oe0")){
        //Serial.println("Disable error message output");
        errorOut = 0;
      }
      else if(!strcmp(inData,"#s00")){
        //errorPrintln("Received Synch token");
        synched = 1;
        Serial.write("#SYNCH00\r\n");
      }
      else{
        inCharCount = 0;
        inData[inCharCount]=0;
  ...
  if ((millis() - timestamp) >= OUTPUT_DATA_INTERVAL) {
    ...
    if(synched){
      if(binaryOutput){
        ...
        binaryFloat pitchBi;
        binaryFloat rollBi;
        yawBi.floatingPoint = TO_DEG(yaw);
        pitchBi.floatingPoint = TO_DEG(pitch);
        rollBi.floatingPoint = TO_DEG(roll);
        Serial.write(yawBi.binary, 4);
        Serial.write(pitchBi.binary, 4);
        Serial.write(rollBi.binary, 4);
      }
      else{
